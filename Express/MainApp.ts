...
const mainApp = (app: Application) => {
  ...
  app.get("/api/v1/read-data", (req: Request, res: Response) => {
    try {
        const user: iData | any = database.map((el:iData)=>{
            return el.course, el.id, el.name, el.time
        })
      const dataPath1 = path.join(__dirname, '../data', './database,json')
      fs.writeFile(dataPath1, JSON.stringify(database), ()=>{
        console.log('reading');
